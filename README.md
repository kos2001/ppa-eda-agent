...
`pipeline/analog/inv/` is `inv.sch` (the cell) → `inv.sym` (generated by
xschem's own `make_symbol`) → `inv_tb.sch` (the testbench, which
instantiates the cell). The generated deck keeps a `%PDK_LIB%` token
...
text, which is a normal way to use xschem but not the whole tool), no
