...
const dict = {
  ...
  sh_lede: {
    en: "The agent system itself, not any one run — same self_improve.py scan you would run in a terminal. Each number is a lever, not a score.",
    ko: "개별 실행이 아니라 에이전트 시스템 자체의 상태입니다 — 터미널에서 돌리는 self_improve.py 스캔과 같은 코드 경로입니다. 각 수치는 점수가 아니라 레버입니다.",
  },
  sh_hl_clear: { en: "Nothing needs attention right now.", ko: "지금 처리할 것이 없습니다." },
  sh_hl_act: { en: "{n} can be re-run as-is", ko: "{n}건은 그대로 재실행하면 됨" },
  sh_hl_judge: { en: "{n} needs a decision", ko: "{n}건은 판단이 필요함" },
  sh_hl_warn: { en: "{n} review(s) cite unverifiable references", ko: "리뷰 {n}건이 확인 불가 참조를 인용" },
