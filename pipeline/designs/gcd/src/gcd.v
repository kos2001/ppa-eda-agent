// Vendored from OpenROAD-flow-scripts (flow/designs/src/gcd/gcd.v).
//
// One mechanical change: 10 lines reading `// verilator-xinit: zeros`
// were deleted. Verilator treats a comment beginning with its own name
// as a directive and rejects this one outright ("Unknown verilator
// comment"), so OpenLane's lint step failed before synthesis started.
// The lines are simulation-only initialisation hints; nothing
// synthesisable was touched and no other edit was made.
//
//-----------------------------------------------------------------------------
// GcdUnit
//-----------------------------------------------------------------------------
...
(
  ...
  always @ (*) begin
    ...
    // Defaults added when vendoring. do_swap and do_sub are assigned
    // only inside the STATE_CALC branch, so Verilator infers a latch
    // for each and OpenLane's lint step stops the flow before synthesis
    // (LINTER_ERROR_ON_LATCH defaults to true; ORFS builds this design
    // without that check).
    //
    // Behaviour-preserving, not a suppression: both signals are read
    // only at lines 300 and 303, inside the same branch that assigns
    // them one line earlier, so the value the latch was holding outside
    // STATE_CALC is never read by anything.
    do_swap = 0;
    do_sub  = 0;
